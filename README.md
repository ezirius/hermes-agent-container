...
This repository builds a small local image derived from the official Hermes Agent container, then runs per-workspace gateway and dashboard pods with repository-owned configuration, wrapper scripts, shell helpers, and shell tests.
...
The image build starts from the official upstream `nousresearch/hermes-agent` image and only adds repo-local customization packages. Today that customization is `nushell`, which is the default shell opened by `hermes-agent-shell`.
...
The wrapper starts a dashboard container with `--host 0.0.0.0` so the published Podman port can reach the containerized dashboard. The published host port is bound to `127.0.0.1` to keep that surface local by default. This is still an explicit local-development tradeoff, not a hardened deployment mode.
