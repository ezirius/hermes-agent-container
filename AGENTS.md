...
- Interactive CLI containers are ephemeral, use the exact name `<image-name>-<workspace>-cli`, share the same `/opt/data` and `/workspace/general` mounts as the persistent runtime, do not join the workspace pod, do not publish ports, and may only remove stale exact-name containers whose workspace mount still matches.
