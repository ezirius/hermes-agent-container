...
- Interactive CLI containers are created first as the temporary exact name `<image-name>-<workspace>-cli`, then renamed to `<image-name>-<workspace>-cli-<12char-container-id>` before attach. They share the same `/opt/data` and `/workspace/general` mounts as the persistent runtime, do not join the workspace pod, do not publish ports, and may only remove stale same-workspace temporary exact-name or renamed session containers.
