...
grep -q 'does not remove existing workspace pods or containers until the replacement gateway and dashboard containers have started successfully' "$ROOT/docs/usage/shared/usage.md"
...
grep -q '^# This script starts saved workspace gateway and dashboard containers, then opens Hermes\.$' "$ROOT/scripts/agent/shared/hermes-agent-run"
