...
test -f "$ROOT/config/patches/apply-hermes-matrix-auto-verification.py"
test -f "$ROOT/config/patches/apply-hermes-matrix-config-overrides.py"
test -f "$ROOT/config/patches/apply-hermes-matrix-upload-filesize.py"
...
grep -q '^  && python3 /tmp/hermes-patches/apply-hermes-matrix-auto-verification.py \\$' "$ROOT/config/containers/Dockerfile"
grep -q '^  && python3 /tmp/hermes-patches/apply-hermes-matrix-config-overrides.py \\$' "$ROOT/config/containers/Dockerfile"
grep -q '^  && python3 /tmp/hermes-patches/apply-hermes-matrix-upload-filesize.py \\$' "$ROOT/config/containers/Dockerfile"
...
grep -q 'self._allowed_users: list\[str\] = \[' "$ROOT/config/patches/apply-hermes-matrix-auto-verification.py"
grep -q 'client.users_for_key_query.update(self._allowed_users)' "$ROOT/config/patches/apply-hermes-matrix-auto-verification.py"
grep -q 'preloaded device keys for verification users' "$ROOT/config/patches/apply-hermes-matrix-auto-verification.py"
grep -q 'client.add_to_device_callback' "$ROOT/config/patches/apply-hermes-matrix-auto-verification.py"
grep -q 'MATRIX_ALLOWED_USERS is not configured' "$ROOT/config/patches/apply-hermes-matrix-auto-verification.py"
grep -q 'extra\["device_id"\] = os.getenv("MATRIX_DEVICE_ID", "")' "$ROOT/config/patches/apply-hermes-matrix-config-overrides.py"
grep -q 'extra\["allowed_users"\] = os.getenv("MATRIX_ALLOWED_USERS", "")' "$ROOT/config/patches/apply-hermes-matrix-config-overrides.py"
grep -q 'filesize=len(data)' "$ROOT/config/patches/apply-hermes-matrix-upload-filesize.py"
...
grep -q 'MATRIX_ALLOWED_USERS' "$ROOT/README.md"
grep -q 'wildcard `\*` is intentionally not supported' "$ROOT/README.md"
grep -q 'does not import or sync room keys' "$ROOT/README.md"
grep -q 'disabled unless `MATRIX_ALLOWED_USERS` is configured' "$ROOT/README.md"
...
grep -q 'MATRIX_ALLOWED_USERS=@trusted-user:matrix.org' "$ROOT/docs/shared/usage.md"
grep -q 'wildcard `\*` is intentionally not supported' "$ROOT/docs/shared/usage.md"
grep -q 'wrapper workspace: this repo’s host-level isolation unit' "$ROOT/docs/shared/usage.md"
grep -q 'does not import or sync room keys' "$ROOT/docs/shared/usage.md"
grep -q 'disabled unless `MATRIX_ALLOWED_USERS` is configured' "$ROOT/docs/shared/usage.md"
...
grep -q '^BASE_ROOT_ABS="\$(normalize_absolute_path "\$(normalize_path "\$HERMES_BASE_ROOT")")"$' "$ROOT/scripts/shared/bootstrap-test"
grep -q '^\[\[ "\$BASE_ROOT_ABS" != "/" \]\] || fail "bootstrap-test refuses to operate with HERMES_BASE_ROOT=/"$' "$ROOT/scripts/shared/bootstrap-test"
grep -q '^\[\[ "\$WORKSPACE_ROOT" == "\$BASE_ROOT_ABS/"\* \]\] || fail "bootstrap-test resolved an unsafe workspace root: \$WORKSPACE_ROOT"$' "$ROOT/scripts/shared/bootstrap-test"
