...
if grep -q 'get_hermes_home' "$ROOT/config/patches/apply-hermes-matrix-store-home.py"; then
  printf 'assertion failed: matrix store patch should not depend on hermes_constants helper imports\n' >&2
  exit 1
fi
grep -q 'Path(os.getenv("HERMES_HOME", Path.home() / ".hermes")) / "matrix" / "store"' "$ROOT/config/patches/apply-hermes-matrix-store-home.py"
...
grep -q '"\$HERMES_IMAGE_NAME" hermes gateway run' "$ROOT/scripts/shared/hermes-start"
grep -q '^exec_podman_interactive_command run "\${RUN_ARGS\[@\]}"$' "$ROOT/scripts/shared/hermes-open"
grep -q '"\$HERMES_IMAGE_NAME" hermes' "$ROOT/scripts/shared/hermes-open"
grep -q '^exec_podman_interactive_command run \\$' "$ROOT/scripts/shared/hermes-shell"
grep -q '"\$HERMES_IMAGE_NAME" /bin/bash' "$ROOT/scripts/shared/hermes-shell"
